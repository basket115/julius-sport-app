...
function normalizeRow(row: any): FeedRow {
  ...
  return {
    ...
    date,
    dateLabel,
 home: cleanStr(row?.home ?? row?.Heimteam),
away: cleanStr(row?.away ?? row?.Gastteam),
homeScore: cleanNum(row?.homeScore ?? row?.Punkte_Heim),
awayScore: cleanNum(row?.awayScore ?? row?.Punkte_Gast),
